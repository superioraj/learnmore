...
* 샘플코드_02- sensor_reading.py 파일
  - 센서(마이크, 온도, 조) 컨트롤 모듈화 코드 
  - 센서 기본 클래스 정의
```
# sensor_base.py
class SensorBase:
...
while True:
    # 마이크 센서값 값 읽기
    mic_values = mic_sensor.read()
    print('Mic_sound_lv: {}'.format(mic_values))
    ...
    print('Light_lv: {}'.format(light_value))
    sleep(2000)
```
...
## 적용하기 샘플코드
* 프롬프트 엔지니어링, 리버스 프롬프트 엔지니어링, 한 문단 명령, 여러개 코드 조합 등 방법을 활용하여 무엇이든 만들기
  - GPT프롬프트 템플릿
    ```
