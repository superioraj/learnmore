...
* 샘플코드_01
  ...
  - GPT 프롬프트 템플릿 입력 내용
    ```
     #GPT 3.5모델로는 아래 '기능'내용을 입력하게 되면, 원하는 코드가 제공 되지 않을 수 있다. 그럴 때는 마이크로 비트 LED 0,0부터 4,4까지 LED좌표를 구체적으로 넣어 기능을 설명하는 것을 추천한다. 나아가 원하는 코드 출력이 되지 않았을 때는 지속적으로 대화로 요청하며 오류가 발생하는 부분 수정 및 원하는 기능 표현을 제시하여 코드를 정교화할 수 있다.
     마이크로 비트 기능: 
     1) 중복 for문 활용
