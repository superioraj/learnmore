...
* [런모어 질문] Chat GPT기능의 커스텀 인스트럭션(Custom instruction)은 어떻게 작성해야 합니까?
  ...
  - 마이크로 비트를 파이썬으로 다루기 위한 '학습자'의 커스텀 인스트럭션 추천 질문
    ...
    목표 (What are some goals you have?): 마이크로비트를 사용하여 어떤 프로젝트나 목표를 달
    성하고자 하시나요?
    
    선호도 정보 관련 질문:
    폼말함 혹은 친근함 (How formal or casual should Chat GPT be?): 마이크로비트와 파이썬 관련 정보를 제공할 때, 공식적이고 학술적인 스타일을 선호하시나요, 아니면 친근하고 대화형 스타일을 선호하시나요?
    ...
    이러한 질문들을 통해 마이크로비트를 파이썬으로 제어하는 학습 활동에 대한 맞춤형 조언과 정보를 제공할 수 있습니다. 사용자의 배경과 선호도에 따라, 더 효과적이고 만족스러운 학습 경험을 제공하는 데 도움이 됩니다.
    ``` 
* 샘플코드_01
  - GPT프롬프트 템플릿
    ```
