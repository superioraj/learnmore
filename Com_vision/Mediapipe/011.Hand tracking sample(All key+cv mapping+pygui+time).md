# .Hand tracking sample(All key+cv mapping+pygui+time)
            ...
            for i, (x, y, w, h) in enumerate(boxes):
                if x < lm_x < x + w and y < lm_y < y + h:
                    if i == 0:
                        ...
                        threading.Thread(target=press_key, args=('right',)).start()
                        time.sleep(0.5)
                    elif i == 2: 
                        threading.Thread(target=press_key, args=('down',)).start()
                        time.sleep(0.5)
                    elif i == 3: 
                        threading.Thread(target=press_key, args=('left',)).start()
                        time.sleep(0.5)
                    elif i == 4: 
                        threading.Thread(target=press_key, args=('a',)).start()
                        time.sleep(0.5)
                    elif i == 5: 
                        threading.Thread(target=press_key, args=('b',)).start()
                        time.sleep(0.5)
